...
namespace dcel {
  ...
  public:
    ...
    /**
     * \fn Surface( unsigned nverts , double* lverts , unsigned nfaces , unsigned* lfaces , unsigned* orient )
     *
     * \brief Creates an instance of this class.
     *
     * \param nverts The number of vertices of this surface.
     ...
     * \param lfaces An array with the vertex identifiers of each face
     * of this surface.
     * \param orient An array  indicating a half-edge incident to each
     * vertex.
     */
    Surface(
	    unsigned nverts ,
    ...
    /**
     * \fn void flip_edge( Edge* e )
     *
     * \brief Flips a given edge.
     *
     * \param e A pointer to the edge to be flipped.
     */
    void flip_edge( Edge* e ) ;


    /**
     * \fn void remove_vertex( Vertex* v )
     *
  ...
  private:
    ...
    /**
     * \fn void create_faces( unsigned nfaces , unsigned* lfaces , HMAP& hmap , VMAP& vmap , unsigned* orient )
     *
     * \brief Creates the faces and half-edges of this surface.
     *
     * \param nfaces The number of faces of this surface.
     ...
     * vertex.
     */
    void create_faces( unsigned nfaces ,
                       unsigned* lfaces ,
                       HMAP& hmap ,
                       VMAP& vmap ,
                       unsigned* orient ) ;
  ...
  /**
   * \fn Surface< VAttrib , FAttrib , EAttrib , HAttrib >::Surface( unsigned nverts , double* lverts , unsigned nfaces , unsigned* lfaces , unsigned* orient )
   *
   * \brief Creates an instance of this class.
   *
   * \param nverts The number of vertices of this surface.
							   ...
							   )
  {
    ...
    /** Create the faces and halfedges and initialize the table. */
    create_faces( nfaces , lfaces , hmap , vmap , orient ) ;
  ...
  /**
   * \fn void Surface< VAttrib , FAttrib , EAttrib , HAttrib >::create_vertices( unsigned nverts , double* lverts , VMAP& vmap )
   *
   * \brief Creates the vertices of this surface.
   *
   * \param nverts The number of vertices of this surface.
  ...
  /**
   * \fn void Surface< VAttrib , FAttrib , EAttrib , HAttrib >::create_faces( unsigned nfaces , unsigned* lfaces , HMAP& hmap , VMAP& vmap )
   *
   * \brief Creates the faces and half-edges of this surface.
   *
   * \param nfaces The number of faces of this surface.
  ...
  /**
   * \fn void Surface< VAttrib , FAttrib , EAttrib , HAttrib >::create_faces( unsigned nfaces , unsigned* lfaces , HMAP& hmap , VMAP& vmap , unsigned* orient )
   *
   * \brief Creates the faces and half-edges of this surface.
   *
   * \param nfaces The number of faces of this surface.
								...
								)
  {
	 ...
	 * Set the half-edge pointer of the vertex pointed by vertex.
	 */
	if ( orient[ lfaces[ j + k ] ] == lfaces[ j + ( ( k + 1 ) % 3 ) ] ) {
	  vertex->set_halfedge( h[ k ] ) ;
	}
      }
  ...
  template <
           ...
           >
  void
  Surface< VAttrib , FAttrib , EAttrib , HAttrib >::create_edges( HMAP& hmap )
  {
    /*
  ...
  /**
   * \fn bool Surface< VAttrib , FAttrib , EAttrib , HAttrib >::is_consistent() const
   *
  ...
  /**
   * \fn void Surface< VAttrib , FAttrib , EAttrib , HAttrib >::flip_edge( Edge* e )
   *
   * \brief Flips a given edge.
   *
   * \param e A pointer to the edge to be flipped.
  ...
  template <
           ...
           >
  void
  Surface< VAttrib , FAttrib , EAttrib , HAttrib >::flip_edge( Edge* e )
  {
    /*
     * Make sure the edge belongs to this surface.
     */
    Halfedge* he = e->get_first_halfedge() ;

    assert( he->get_face()->get_surface() == this ) ;

    /*
     * Update the half-edge pointers of the edge vertices.
     */
    if ( he->get_origin()->get_halfedge() == he ) {
      he->get_origin()->set_halfedge( he->get_mate()->get_next() ) ;
    }

    if ( he->get_mate()->get_origin()->get_halfedge() == he->get_mate() ) {
      he->get_mate()->get_origin()->set_halfedge( he->get_next() ) ;
    }

   /*
    * Get  half-edges with  origin at  the vertices  opposite  to the
    * given edge in the two faces incident with the edge.
    */
    Halfedge* he1 = he->get_prev() ;
    Halfedge* he2 = he->get_mate()->get_prev() ;

    /*
     * Update the vertex  pointer of each half-edge of  the edge being
     * flipped.
     */
    he->set_origin( he2->get_origin() ) ;
    he->get_mate()->set_origin( he1->get_origin() ) ;

    /*
     * Update the half-edge cycles of both faces.
     */
    he1->set_next( he2->get_prev() ) ;
    he2->get_prev()->set_prev( he1 ) ;

    he1->get_prev()->set_prev( he2 ) ;
    he2->set_next( he1->get_prev() ) ;

    he->get_next()->set_next( he->get_mate() ) ;
    he->get_mate()->set_prev( he->get_next() ) ;

    he->set_next( he1 ) ;
    he1->set_prev( he ) ;

    he->get_mate()->set_next( he2 ) ;
    he2->set_prev( he->get_mate() ) ;

    he->set_prev( he1->get_next() ) ;
    he->get_prev()->set_next( he ) ;

    /*
     * Update the face pointers of all half-edges.
     */
    he1->get_next()->set_face( he1->get_face() ) ;
    he2->get_next()->set_face( he2->get_face() ) ;

    /*
     * Update the half-edge pointer of both faces.
     */
    he1->get_face()->set_halfedge( he ) ;
    he2->get_face()->set_halfedge( he->get_mate() ) ;
  ...
  /**
   * \fn void Surface< VAttrib , FAttrib , EAttrib , HAttrib >::remove_vertex( Vertex* v )
   *
   * \brief Removes a vertex and all of its incident faces.
   *
   * \param v A pointer to the vertex to be removed.
  ...
  template <
           ...
           >
  void
  Surface< VAttrib , FAttrib , EAttrib , HAttrib >::remove_vertex( Vertex* v )
  {
    /*
     * Collect  all faces  and  edges  incident to  the  vertex to  be
     * removed, as well  as all half-edges whose origin  vertex is the
     * vertex to be removed.
     */
    std::list< Face* > lfaces ;
    std::list< Edge* > ledges ;
    std::list< Halfedge* > lhedges ;

    Halfedge* he = v->get_halfedge() ;
    Halfedge* heaux = he ;
    do {
      lfaces.push_back( heaux->get_face() ) ;
      ledges.push_back( heaux->get_edge() ) ;
      lhedges.push_back( heaux ) ;
      heaux = heaux->get_prev()->get_mate() ;
    }
    while( heaux != he ) ;

    assert(
	   ( lfaces.size() == lhedges.size() ) &&
	   ( lfaces.size() >= 3 )
	  ) ;

    /*
     * Update half-edge and vertex pointers.
     */
    typename std::list< Halfedge* >::iterator hit = lhedges.begin() ;
    he = heaux = *hit ;
    ++hit ;
    do {
      /*
       * Update the  pointers to previous  and next half-edges  of all
       * half-edges that belong to the edges of the link of the vertex
       * to be removed.
       */
      ( *hit )->get_next()->set_prev( heaux->get_next() ) ;
      heaux->get_next()->set_next( ( *hit )->get_next() ) ;

      /*
       * Update  the half-edge  pointer of  the origin  vertex  of all
       * half-edges that belong to the edges of the link of the vertex
       * to be removed.
       */
      heaux->get_next()->get_origin()->set_halfedge( heaux->get_next() ) ;

      /*
       * Update the face pointer of  all half-edges that belong to the
       * edges of the link of the vertex to be removed.
       */
      heaux->get_next()->set_face( he->get_face() ) ;

      heaux = *hit ;

      ++hit ;
    }
    while ( hit != lhedges.end() ) ;

    /*
     * Update the previous  pointer and next pointer of  the first and
     * last  half-edges (respectively)  of edges  of the  link  of the
     * vertex to be removed.
     */
    he->get_next()->set_prev( heaux->get_next() ) ;
    heaux->get_next()->set_next( he->get_next() ) ;

    /*
     * Update the half-edge  pointer of the origin vertex  of the last
     * half-edge belonging to an edge of  the link of the vertex to be
     * removed.
     */
    heaux->get_next()->get_origin()->set_halfedge( heaux->get_next() ) ;

    /*
     * Update the face  pointer of the last half-edge  belonging to an
     * edge of the link of the vertex to be removed.
     */
    heaux->get_next()->set_face( he->get_face() ) ;

    /*
     * Update the half-edge pointer of  the face that will replace all
     * deleted faces.
     */
    he->get_face()->set_halfedge( heaux->get_next() ) ;

    /*
     * Remove all half-edges of the star of "v".
     */
    while ( !lhedges.empty() ) {
      Halfedge* h = lhedges.front() ;
      lhedges.pop_front() ;
      delete h->get_mate() ;
      delete h ;
    }
    ...
    /*
     * Remove all edges of the star of "v".
     */
    while ( !ledges.empty() ) {
      Edge* e = ledges.front() ;
      ledges.pop_front() ;
      _list_of_edges.remove( e ) ;
      delete e ;
    }

    /*
     * Remove all  faces of the star  of "v", except for  the one that
     * will replace all the others.
     */
    lfaces.pop_front() ;
